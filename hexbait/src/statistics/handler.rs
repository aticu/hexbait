...
/// The size of an entropy sample window.
const ENTROPY_SAMPLE_SIZE: Len = Len::from(1024);

/// An entropy value for a window.
struct EntropyValue {
...
struct CalculationResult {
    /// Comptued entropy values.
    ///
    /// Sorted by `window`.
    entropy_values: Vec<EntropyValue>,
    /// The computed statistics for the selected window.
    statistics: Statistics,
}

/// The quality of a returned entropy value.
pub enum EntropyQuality {
    /// The entropy value is estimated.
    Estimated,
    /// The entropy value is accurately computed.
    Accurate,
}
...
impl EntropyQuality {
    pub fn is_estimated(&self) -> bool {
        match self {
            EntropyQuality::Estimated => true,
            EntropyQuality::Accurate => false,
        }
    }
}
...
impl StatisticsHandler {
    ...
    pub fn get_bigram_statistics(&self, window: Window) -> (Statistics, f32) {
    ...
    pub fn get_entropy(&self, window: Window) -> Option<(Entropy, EntropyQuality)> {
        ...
        {
            Ok(mut index) => {
                while index > 0 && result.entropy_values[index - 1].window.start() == window.start()
                {
                    index -= 1;
                }
                index
        ...
        if let Some(value) = result.entropy_values.get(index)
            && value.window == window
        {
            return Some((value.entropy, EntropyQuality::Accurate));
        }
        ...
        for sample in result.entropy_values[index..].iter().take(5) {
            if sample.window == window {
                return Some((sample.entropy, EntropyQuality::Accurate));
            }
        ...
        if count == 0 {
            ...
            Some((
                Entropy((sum / count) as u8),
                if window.size() == ENTROPY_SAMPLE_SIZE {
                    EntropyQuality::Accurate
                } else {
                    EntropyQuality::Estimated
                },
            ))
        }
    }
...
fn raw_bin_size_to_bin_size(raw_bin_size: Len) -> Len {
    ...
    Len::from(rounded_bin_size).max(ENTROPY_SAMPLE_SIZE)
}
