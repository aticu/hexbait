...
impl Provenance {
    ...
    pub fn from_window(window: Window) -> Provenance {
        let mut byte_ranges = RangeSetBlaze::new();
        if !window.is_empty() {
            byte_ranges.ranges_insert(window.start()..=window.end() - 1);
        }
...
impl From<Window> for Provenance {
    fn from(value: Window) -> Self {
        Provenance::from_window(value)
    }
}
